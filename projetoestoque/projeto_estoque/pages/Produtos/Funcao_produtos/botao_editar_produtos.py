...
def editar_produtos_do_estoque():
    ...
    if st.button("Próximo"):
        ...
        for i, info in enumerate(produtos2):
            nome_prod = st.text_input(f"*Nome do Produto*: {info[2]}", key=f"nome_produto_{i}")
            marca_prod = st.text_input(f"*Marca do Produto*: {info[1]}", key=f"marca_produto_{i}")
            unidade_medida = st.text_input(f"*Unidade de Medida*: {info[0]}", key=f"unidade_medida_{i}")
            quantidade_estoque = st.text_input(f"*Quantidade em Estoque*: {info[3]}", key=f"quantidade_estoque_{i}")
